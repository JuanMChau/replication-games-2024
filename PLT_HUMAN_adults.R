...
levels(bigDF.adults$ID) <- seq(126+1,126+length(unique(bigDF.adults$ID)))
...
## combine ##
bigDF <- rbind.data.frame(bigDF.adults)
...
select_group=c('adults_up','adults_inv')
...
flex_group_rej_summary<-flextable::add_header_row(flex_group_rej_summary,values=c("ID",rep(c("tested", "included","rejected","%rejected"),2)))
(flex_group_rej_summary<-flextable::theme_booktabs(flextable::add_header_row(flex_group_rej_summary,colwidths=c(1,4,4),values=c('',select_group))))
#flextable::save_as_docx(flex_group_rej_summary,path='trial_summaries/group_rej_summary_adult.docx')
# write.csv(group_rej_summary,file='trial_summaries/group_rej_summary.csv',row.names=F)

psych::describe(unlist(group_rej_summary_total%>%select(ends_with('%rejected')&starts_with(c('adults_up')))%>%filter(rownames(group_rej_summary_total)%notin%c('TOTAL','M','SD'))))
psych::describe(unlist(group_rej_summary_total%>%select(ends_with('%rejected')&starts_with(c('adults_inv')))%>%filter(rownames(group_rej_summary_total)%notin%c('TOTAL','M','SD'))))
#
#
#### INFORMATIVE TIME WINDOW ####
...
alpha=.01
threshold=qt(p=alpha/2,df=23,lower.tail=FALSE) # two-tailed
# # adults
...
### ANALYSIS PARAMETERS ###
tbin=1
n_samples=500 # <- TBD: Modify this to 10k?
alpha=.10
threshold=qt(p=alpha/2,df=23,lower.tail=FALSE) # two-tailed
gap=0
